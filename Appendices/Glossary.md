...
see [→Device operating mode](#h-device-operating-mode), [→Storage control mode](#h-storage-control-mode).
...
One of [→Storage operating modes](#h-storage-control-mode)
...
Any device connected to the [Unwaste Robot](#h-unwaste-robot) (either via cloud or locally) that can be monitored and managed.
...
Energy usage forecast is required for storage control. See [→Storage control algorithm](#h-storage-control-algorithm).
...
For details, see [→Storage control algorithm](#h-storage-control-algorithm).
...
An override forces a device to operate in a selected [→Device operating mode](#h-device-operating-mode) for a specific time period.
...
The only requirement is that the [Unwaste Robot](#h-unwaste-robot) can read its data.
...
Generating savings on your electricity bill is the main goal of the [Unwaste Robot](#h-unwaste-robot).
...
**Note 2.** The [Unwaste Robot](#h-unwaste-robot) only sends price-based signals. Final decisions are made by devices themselves, especially temperature-controlled devices such as heat pumps or air conditioning systems.
...
A schedule is a set of rules that override default device behavior (see [→Device control algorithm](#h-device-control-algorithm)).

Each rule defines:

* when it applies
* which [→Device operating mode](#h-device-operating-mode) is used

Each device can have one schedule per [→Profile](#h-profile).
...
Both standalone storages and storages integrated with hybrid inverters are supported, provided the [Unwaste Robot](#h-unwaste-robot) can read and control them.
...
The [Unwaste Robot](#h-unwaste-robot) may charge storage from the grid only when:
...
* storage supports [→"Force charge" mode](#h-force-charge-mode-storages)
* [→Solar production forecast](#h-solar-production-forecast) configured
* [→Energy usage forecast](#h-energy-usage-forecast) configured

The [Unwaste Robot](#h-unwaste-robot) checks:
...
See [→ Unwaste Robot](#h-unwaste-robot)
...
One of [→ Device operating modes ](#h-device-operating-mode)and [→Storage operating modes](#h-storage-control-mode)

In this mode, the device or storage is not managed by the [Unwaste Robot](#h-unwaste-robot) and operates according to its own rules.
